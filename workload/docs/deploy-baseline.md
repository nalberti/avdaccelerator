...
- **Session hosts** blade
  ...
  - **Zero trust disk configuration** - Check the box to enable the zero trust configuration on the session host disks to ensure that network access is disabled on the disk.
  - **Encryption At Host disk configuration** - Check the box to enable the Encryption At Host (dm-crypt) configuration on the session host disks to ensure all the disks are encrypted, the OS and data disks are protected with double encryption with a customer managed key. If Encryption At Host is enabled, Azure Disk Encryption be disabled.
  - **Azure Disk Encryption configuration** - Check the box to enable the Azure Disk Encryption on the session host disks to ensure all the disks are encrypted, the OS and data disks are protected with double encryption with a customer managed key. If Azure Disk Encryption is enabled, Encryption At Host must be disabled.
